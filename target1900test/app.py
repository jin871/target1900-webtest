...
# 苦手リスト(CSV)への保存関数
def save_weak_words(words, filename='weak_target.csv'):
    try:
...
def get_words():
    ...
    # ターゲット1900用のファイル名に変更
    filename = 'weak_target.csv' if mode == 'weak' else 'target1900.csv'
    all_words = load_words(filename)
    
    if not all_words:
        return jsonify({"error": f"データが見つかりません。{filename}を確認してください。"})
        
    # 苦手モードの時は範囲の絞り込みをせず、リスト全体を対象にする
    if mode == 'weak':
...
def save_results():
    ...
    # 苦手リストのファイル名を変更
    weak_words = load_words('weak_target.csv')
    ...
    save_weak_words(list(weak_dict.values()), 'weak_target.csv')
